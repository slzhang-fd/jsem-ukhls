...
double log_ei_tp_fp_c(double x, void* params){
  ...
  arma::vec tmp1 = tmp % d->ytp_fp_i - arma::log(1.0 + arma::exp(tmp));
...
void sample_e12_new(arma::mat &e12, const arma::mat &ytp, const arma::mat &yfp, const arma::uvec &xi,
  ...
  Sigma(0,1) = rho * std::sqrt(sig2_tp * sig2_fp);
  Sigma(1,0) = Sigma(0,1);
  e12.rows(xi_1_loc) = arma::mvnrnd(arma::zeros(2), Sigma, xi_1_loc.n_elem).t();
  
  arma::uvec xi_2_loc = arma::find(xi == 2);
  for(auto& ii : xi_2_loc){
    double xsamp[100];
    // sample e(:,1) or e_fp
    double xprev = e12(ii,1);
    log_ei_tp_fp_params log_ei_fp_data;
    log_ei_fp_data.alpha_tp_fp_i = alpha_fp.row(ii).t();
    log_ei_fp_data.beta_tp_fp_i = beta_fp.row(ii).t();
    log_ei_fp_data.b_tp_fp = b_fp;
    ...
    e12(ii,1) = xsamp[0];
    e12(ii,0) = std::sqrt(sig2_tp / sig2_fp) * rho * xsamp[0] + std::sqrt((1.0 - rho*rho)*sig2_tp) * arma::randn(); // place holder
  }
  ...
  for(auto& ii : xi_3_loc){
    ...
    e12(ii,0) = xsamp[0];
    e12(ii,1) = std::sqrt(sig2_fp / sig2_tp) * rho * xsamp[0] + std::sqrt((1.0 - rho*rho)*sig2_fp) * arma::randn();;
  }
  
  arma::uvec xi_4_loc = arma::find(xi == 4);
  for(auto& ii : xi_4_loc){
    double xsamp[100];
    // sample e12(ii,0) or e_tp
    double xprev = e12(ii, 0);
    log_e12_i_params log_e12_i_data;
    log_e12_i_data.ytp_fp_i = ytp.row(ii).t();
    log_e12_i_data.alpha_tp_fp_i = alpha_tp.row(ii).t();
